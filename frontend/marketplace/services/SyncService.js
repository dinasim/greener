...
class SyncService {
  ...
  async init() {
    try {
      ...
      if (queueJson) { this.syncQueue = JSON.parse(queueJson); }
      this.setupNetworkMonitoring();
      this.processSyncQueue();
    } catch (error) { console.error('Error initializing sync service:', error); }
  }
  setupNetworkMonitoring() {
    NetInfo.fetch().then(state => { this.isOnline = state.isConnected; });
    NetInfo.addEventListener(state => {
      const wasOffline = !this.isOnline;
      this.isOnline = state.isConnected;
      if (wasOffline && this.isOnline && this.syncQueue.length > 0) { this.processSyncQueue(); }
      this.notifyListeners({ type: 'CONNECTION_CHANGE', isOnline: this.isOnline });
    });
  }
      ...
      while (this.syncQueue.length > 0) {
        ...
        try {
          ...
          switch (operation.type) {
            case 'CREATE_PLANT': success = await this.syncCreatePlant(operation); break;
            case 'UPDATE_PLANT': success = await this.syncUpdatePlant(operation); break;
            case 'DELETE_PLANT': success = await this.syncDeletePlant(operation); break;
            case 'TOGGLE_WISHLIST': success = await this.syncToggleWishlist(operation); break;
            case 'UPDATE_PROFILE': success = await this.syncUpdateProfile(operation); break;
            case 'SEND_MESSAGE': success = await this.syncSendMessage(operation); break;
            default: console.warn(`Unknown operation type: ${operation.type}`); success = true;
          }
          if (success) {
            ...
            if (failedOp.retryCount < 5) { this.syncQueue.push(failedOp); }
  ...
  async updateLocalPlantId(tempId, realId) {
    try {
      for (const op of this.syncQueue) {
        if (op.data?.plantId === tempId) { op.data.plantId = realId; }
      }
      await AsyncStorage.setItem(this.SYNC_QUEUE_KEY, JSON.stringify(this.syncQueue));
    } catch (error) { console.error('Error updating local plant ID:', error); }
  }
  ...
  notifyListeners(event) {
    for (const listener of this.syncListeners) {
      try { listener(event); } 
      catch (error) { console.error('Error in sync listener:', error); }
    }
  }
  getSyncStatus() {
    return { queueLength: this.syncQueue.length, isOnline: this.isOnline, isSyncing: this.isSyncing };
  }
  sync() {
